...
| contactEmail 편집 UI | 모달 방식 → **전체 필드 편집 모달 (CompanyForm 재사용)** |
...
- 트리거: "정보 수정" 버튼
- **변경 (2026-04-29):** contactEmail 전용 → CompanyForm을 `variant="plain"` 모달로 재사용하여 전체 필드 편집 가능
- CompanyForm에 `onSuccess`/`onCancel`/`variant` props 추가 (모달/standalone 양쪽 지원)
- 모달 크기: `max-w-4xl`, 필드 간격 `gap-x-6 gap-y-5`
- base-ui Select 포털 컨텍스트에서 초기값 라벨 미표시 → SelectValue에 명시적 children 전달로 해결
...
ℹ️ 본 시뮬레이터는 고용허가제 E-9 (일반 외국인) 비자 기준입니다.
   ...
   E-7-4 (숙련기능) 전환은 별도 추천서 발급 절차이며 본 도구 범위 외입니다.
```
...
- [x] 사이드바 6개 카테고리 + 메뉴 7개 정상 표시
- [x] /settings/company → 사업장 선택 드롭다운 변경 시 자동 콘텐츠 갱신
- [x] /settings/company → "정보 수정" 모달에서 **전체 필드** 편집 + 저장 (CompanyForm 재사용)
- [x] /settings/notifications → 알림 시점 토글 localStorage 저장/복원
- [x] /settings/notifications → 수동 트리거 버튼 동작 + 토스트
- [x] /settings/notifications → 발송 기록 테이블 표시 (MSW 목업)
- [x] 시뮬레이터 라벨 "E-9 고용허가 시뮬레이터" + 안내 박스 표시
- [x] 대시보드 타임라인 → 알림 설정 딥링크
- [x] Vitest 681건 통과
- [x] 기존 페이지 regression 없음

### 추가 DoD (2026-04-29 구현 중 발견)

- [x] 시뮬레이션 폼 DEDUCTION 그룹 렌더링 (라벨/색상/부호) 정상 표시
- [x] `deductionScore` 필드 제거 + `ScoringPolicyGroup` metadata 연동
- [x] base-ui SelectValue 한글 라벨 표시 (전체 9건)
- [x] `VISA_TYPE_LABELS` 법무부 정식 체류자격 명칭 통일
- [x] 알림 토글 "OVERDUE" 영문 라벨 → "기한 초과" 한글화
