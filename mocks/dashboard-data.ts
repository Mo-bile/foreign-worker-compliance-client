import type { DashboardRawResponse } from "@/types/dashboard";

export const mockDashboardRaw: DashboardRawResponse = {
  stats: {
    totalWorkers: 12,
    visaBreakdown: [
      { visaType: "E9", count: 8 },
      { visaType: "H2", count: 3 },
      { visaType: "E7", count: 1 },
    ],
    insuranceRate: 91.7,
    upcomingDeadlines: 5,
    deadlineBreakdown: { d7: 2, d30: 3 },
    urgentActions: 3,
    urgentBreakdown: { visa: 1, insurance: 2 },
  },
  alerts: [
    {
      deadlineId: 1,
      workerId: 1,
      workerName: "Nguyen Van A",
      deadlineType: "VISA_EXPIRY",
      status: "URGENT",
      dDay: -4,
      dueDate: "2026-04-02",
      description: "E-9 비자가 2026.03.28에 만료됩니다. 즉시 연장 신청이 필요합니다.",
    },
    {
      deadlineId: 2,
      workerId: 2,
      workerName: "Pham Thi B",
      deadlineType: "HEALTH_INSURANCE_ENROLLMENT",
      status: "OVERDUE",
      dDay: 0,
      dueDate: "2026-03-29",
      description: "입사 후 14일이 경과했으나 건강보험 취득신고가 완료되지 않았습니다.",
    },
    {
      deadlineId: 3,
      workerId: 3,
      workerName: "Rahman C",
      deadlineType: "CONTRACT_RENEWAL",
      status: "APPROACHING",
      dDay: -21,
      dueDate: "2026-04-19",
      description: "근로계약 만료 30일 전입니다. 갱신 절차를 진행하세요.",
    },
  ],
  visaDistribution: [
    { visaType: "E9", count: 8, percentage: 66.7 },
    { visaType: "H2", count: 3, percentage: 25.0 },
    { visaType: "E7", count: 1, percentage: 8.3 },
  ],
  insuranceSummary: [
    { insuranceType: "NATIONAL_PENSION", mandatoryCount: 10, totalWorkers: 12 },
    { insuranceType: "HEALTH_INSURANCE", mandatoryCount: 12, totalWorkers: 12 },
    { insuranceType: "EMPLOYMENT_INSURANCE", mandatoryCount: 9, totalWorkers: 12 },
    { insuranceType: "INDUSTRIAL_ACCIDENT", mandatoryCount: 12, totalWorkers: 12 },
  ],
  complianceScore: {
    score: 73,
    breakdown: [
      { category: "INSURANCE", score: 92 },
      { category: "DEADLINE", score: 65 },
    ],
  },
  aiInsight: {
    content:
      "**현재 상태 요약:** 귀사는 외국인 근로자 12명을 고용 중이며, 전체 4대보험 의무 적용률은 91.7%입니다. E-9 비자 근로자 8명 중 국민연금 취득신고 지연 2건이 확인됩니다.\n\n**리스크:** Nguyen Van A 근로자의 비자 만료가 **4일 후**로 임박해 있습니다. 연장 미신청 시 불법체류 전환 및 사업주 과태료(최대 2,000만원) 리스크가 있습니다.\n\n**권장 조치:** ① 비자 연장 즉시 신청 ② 국민연금 취득신고 지연 2건 처리 ③ Rahman C 근로자 근로계약 갱신 준비",
    generatedAt: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString(),
  },
  upcomingDeadlines: [
    {
      deadlineId: 1,
      workerId: 1,
      workerName: "Nguyen Van A",
      visaType: "E9",
      deadlineType: "VISA_EXPIRY",
      status: "URGENT",
      dDay: -4,
      dueDate: "2026-04-02",
    },
    {
      deadlineId: 2,
      workerId: 2,
      workerName: "Pham Thi B",
      visaType: "E9",
      deadlineType: "HEALTH_INSURANCE_ENROLLMENT",
      status: "OVERDUE",
      dDay: 0,
      dueDate: "2026-03-29",
    },
    {
      deadlineId: 4,
      workerId: 4,
      workerName: "Rahman C",
      visaType: "H2",
      deadlineType: "CONTRACT_RENEWAL",
      status: "APPROACHING",
      dDay: -21,
      dueDate: "2026-04-19",
    },
    {
      deadlineId: 5,
      workerId: 5,
      workerName: "Li Wei D",
      visaType: "E7",
      deadlineType: "VISA_EXPIRY",
      status: "APPROACHING",
      dDay: -45,
      dueDate: "2026-05-13",
    },
    {
      deadlineId: 6,
      workerId: 6,
      workerName: "Tran E",
      visaType: "E9",
      deadlineType: "CHANGE_REPORT",
      status: "APPROACHING",
      dDay: -58,
      dueDate: "2026-05-26",
    },
  ],
};
