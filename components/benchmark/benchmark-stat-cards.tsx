  ...
  return [
    {
      label: "임금 포지셔닝",
      value: wageAnalysis
        ? `${wageAnalysis.companyAvgWage ?? "—"}만원`
        : "—",
      sub: wageAnalysis
        ? `${wageAnalysis.companyBracket ?? "—"} 구간`
        : "미입력",
    ...
    },
    {
      label: "고용 안정성",
      value:
        stabilityAnalysis?.turnoverRate != null
          ? `${stabilityAnalysis.turnoverRate}%`
          : "—",
      sub: stabilityAnalysis
        ? `최근 1년 퇴사율 (${stabilityAnalysis.terminationCount}명/${stabilityAnalysis.foreignWorkerCount}명)`
        : "미입력",
      accentColor: "border-t-[oklch(0.65_0.18_85)]",
      bgColor: "bg-[oklch(0.95_0.03_85)]",
      icon: "📊",
    },
    {
      label: "관리 수준",
      ...
      sub: (() => {
        ...
        return failedRequired > 0
          ? `법적의무 ${failedRequired}건 미충족`
          : "전체 충족";
      })(),
      accentColor: "border-t-[oklch(0.52_0.15_155)]",
      bgColor: "bg-[oklch(0.95_0.03_155)]",
      icon: "✅",
    },
    {
      label: "지역·업종",
      ...
      accentColor: "border-t-[oklch(0.55_0.18_300)]",
      bgColor: "bg-[oklch(0.95_0.03_300)]",
      icon: "📍",
    },
  ];
}
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {cards.map((card) => (
        <Card
          key={card.label}
          className={`border-t-3 ${card.accentColor} gap-0 py-4`}
        >
