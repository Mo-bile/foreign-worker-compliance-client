...
describe("DeadlinesPage 쿼리파라미터 필터", () => {
  afterEach(() => {
    mockSearchParams.delete("type");
    mockUseCompanyContext.mockReturnValue({ selectedCompanyId: 1, companies: [] });
  });

  it("상단에 기한 관리 안내 문구가 표시된다", () => {
    render(<DeadlinesPage />, { wrapper: createWrapper() });

    expect(screen.getByRole("heading", { name: "기한 관리" })).toBeInTheDocument();
    ...
    expect(screen.getByText(/처리한 항목은 완료 처리해 목록에서 상태를 관리/)).toBeInTheDocument();
  });
