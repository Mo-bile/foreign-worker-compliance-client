...
describe("benchmarkResponseSchema", () => {
  it("정상 데이터를 파싱한다", () => {
    const data = {
      ...
      stabilityAnalysis: {
        ...
        terminationReasons: {
          lowWage: 24.5, companyIssue: 23.1, dangerous: 13.9,
          betterJob: 10.2, environment: 6.4, wageDelay: 5.4, other: 16.5,
        },
      },
      managementCheck: {
        totalItems: 10, passedItems: 8, score: 80,
        items: [{ category: "보험", label: "4대보험", passed: true, required: true }],
      },
      positioningAnalysis: {
        region: "경기도", industryCategory: "식료품제조업",
        regionalTotal: 245000, industryTotal: 92386,
        companyForeignWorkerCount: 12, companyShare: 0.013, sizeCategory: "소규모",
      },
    };
  ...
  it("nullable 축(wage, stability)이 null이면 파싱 성공", () => {
    const data = {
      id: 2, companyId: 1, analyzedAt: "2026-04-11T09:00:00Z",
      managementScore: 80, aiReport: "리포트",
      wageAnalysis: null, stabilityAnalysis: null,
      managementCheck: {
        totalItems: 10, passedItems: 8, score: 80,
        items: [{ category: "보험", label: "4대보험", passed: true, required: true }],
      },
