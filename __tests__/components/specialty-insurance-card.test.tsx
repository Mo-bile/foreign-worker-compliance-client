...
describe("SpecialtyInsuranceCard", () => {
  it("E9_비자에_전용보험_카드를_렌더링한다", () => {
    render(<SpecialtyInsuranceCard visaTypeCode="E9" deadlines={[]} />);
    expect(screen.getByText("전용보험 (E-9/H-2)")).toBeInTheDocument();
  });

  it("H2_비자에_전용보험_카드를_렌더링한다", () => {
    render(<SpecialtyInsuranceCard visaTypeCode="H2" deadlines={[]} />);
    expect(screen.getByText("전용보험 (E-9/H-2)")).toBeInTheDocument();
  });

  it("F2_비자에_렌더링하지_않는다", () => {
    const { container } = render(<SpecialtyInsuranceCard visaTypeCode="F2" deadlines={[]} />);
  ...
  it("사업주_의무에_데드라인이_있으면_상태를_표시한다", () => {
    ...
    render(<SpecialtyInsuranceCard visaTypeCode="E9" deadlines={deadlines} />);
